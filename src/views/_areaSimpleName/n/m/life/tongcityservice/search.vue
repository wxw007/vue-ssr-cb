...
<script>
    ...
    import tongcityServiceSearch from "@/store/modules/m/life/tongcityservice/search.js"
    ...
    export default {
        ...
        asyncData({ store, route, params, query }) {
            return store.dispatch("tongcityServiceSearch/fetchData");
        },
        computed: {
            ...mapState("tongcityServiceSearch", ["list"])
        },
        components: {
            list,
            searchBox
        ...
        methods: {
            loadMore(callback) {
                callback("tongcityServiceSearch/fetchMore", this.list);
            }
        }
    }
</script>